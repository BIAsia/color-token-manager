...
function GetAllLinks() {
    sysLinks = [];
    const defaultPaintStyles = allPaintStyles.filter(style => style.name.startsWith('[' + defaultThemeName + ']'));
    const RefPaintStyles = defaultPaintStyles.filter(style => style.name.startsWith('[' + defaultThemeName + ']' + '/ref'));
    const SysPaintStyles = defaultPaintStyles.filter(style => style.name.startsWith('[' + defaultThemeName + ']' + '/sys'));
...
function ComparePaints(paintA, paintB) {
    if (paintA == null || paintB == null)
        return false;
    if (paintA.opacity != paintB.opacity)
        return false;
